...
import { DispatchAssignmentDto, DispatchAssignmentMapper } from '../mappers/dispatch-assignment.mapper';
...
export class DispatchAssignmentHttpRepository implements DispatchAssignmentRepository {
  ...
  async findById(id: string): Promise<DispatchAssignment | null> {
    try {
      const dto = await firstValueFrom(
        this.http.get<DispatchAssignmentDto>(`${this.baseUrl}/${id}`)
      );
      return DispatchAssignmentMapper.toDomain(dto);
    } catch (error) {
      console.error(`Failed to fetch dispatch assignment with ID ${id}:`, error);
      return null;
    }
  }
  ...
  async findAll(filters?: DispatchAssignmentFilters): Promise<DispatchAssignment[]> {
    ...
    const dtos = await firstValueFrom(
      this.http.get<DispatchAssignmentDto[]>(this.baseUrl, { params })
    );
  ...
  async findByTechnicianAndTimeSlot(
    ...
    const dtos = await firstValueFrom(
      this.http.get<DispatchAssignmentDto[]>(`${this.baseUrl}/conflicts`, { params })
    );
    return dtos.map(DispatchAssignmentMapper.toDomain);
  }
